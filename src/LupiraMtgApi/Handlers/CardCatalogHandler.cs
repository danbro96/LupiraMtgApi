...
public sealed class CardCatalogHandler
{
    ...
    public CardCatalogHandler(LupiraMtgDbContext db, CardPrintingMapper mapper)
    {
        _db = db;
        _mapper = mapper;
    }
    ...
    public async Task<Ok<CardListResponse>> ListCardsAsync(CardListQuery query, CancellationToken ct)
    {
        ...
        return TypedResults.Ok(new CardListResponse { Results = results, Total = total });
    }
    ...
    private sealed record CardListFilters(
        ...
        int Skip)
    {
        public static CardListFilters From(CardListQuery q)
        {
    ...
    public async Task<Results<Ok<CardPrintingListResponse>, NotFound>> ListPrintingsAsync(string oracleId, CancellationToken ct)
    {
        ...
        return TypedResults.Ok(new CardPrintingListResponse { Results = results });
    }

    public async Task<Results<Ok<CardPrintingResponse>, NotFound>> GetPrintingAsync(
        string oracleId,
        string printingId,
