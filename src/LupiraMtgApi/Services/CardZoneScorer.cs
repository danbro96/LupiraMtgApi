...
public sealed class CardZoneScorer
{
    ...
    private readonly ScanScoringOptions options;
    ...
    private async Task ScoreNameAsync(string text, Dictionary<string, PrintingZoneScores> byPrinting, CancellationToken ct)
    {
        ...
        var rows = await this.db.CardPrintings
            ...
            .Where(x => x.Score > this.options.NameCutoff)
            .OrderByDescending(x => x.Score)
            .Take(this.options.NameTopK)
            .ToListAsync(ct);
    ...
    private async Task ScoreTypeLineAsync(string text, Dictionary<string, PrintingZoneScores> byPrinting, CancellationToken ct)
    {
        ...
        var rows = await this.db.CardPrintings
            ...
            .Where(x => x.Score > this.options.TypeLineCutoff)
            .OrderByDescending(x => x.Score)
            .Take(this.options.TypeLineTopK)
            .ToListAsync(ct);
    ...
    private async Task ScoreRulesTextAsync(string text, Dictionary<string, PrintingZoneScores> byPrinting, CancellationToken ct)
    {
        ...
        var rows = await this.db.CardPrintings
            ...
            .Where(x => x.Score > this.options.RulesTextCutoff)
            .OrderByDescending(x => x.Score)
            .Take(this.options.RulesTextTopK)
            .ToListAsync(ct);
    ...
    private async Task ScoreBottomMetadataAsync(string text, Dictionary<string, PrintingZoneScores> byPrinting, CancellationToken ct)
    {
        ...
        {
            var tier1 = await this.db.CardPrintings
                .AsNoTracking()
        ...
        if (rarityLetter is not null)
        {
            ...
            {
                var tier2 = await this.db.CardPrintings
                    .AsNoTracking()
        ...
        {
            var candidateIds = byPrinting.Keys.ToList();
            var tier3 = await this.db.CardPrintings
                .AsNoTracking()
    ...
    private double ComputeAggregate(PrintingZoneScores scores, ZoneWeights weights)
    {
        ...
        {
            sum += (this.options.NameWeight / weights.TotalPresent) * scores.NameScore;
        }

        if (weights.TypeLinePresent)
        {
            sum += (this.options.TypeLineWeight / weights.TotalPresent) * scores.TypeLineScore;
        }

        if (weights.RulesTextPresent)
        {
            sum += (this.options.RulesTextWeight / weights.TotalPresent) * scores.RulesTextScore;
        }

        if (weights.PowerToughnessPresent)
        {
            sum += (this.options.PowerToughnessWeight / weights.TotalPresent) * scores.PowerToughnessScore;
        }

        if (weights.BottomMetadataPresent)
        {
            sum += (this.options.BottomMetadataWeight / weights.TotalPresent) * scores.BottomMetadataScore;
        }
    ...
    private ZoneWeights WeightsForPresentZones(CardZones zones)
    {
        ...
        if (w.NamePresent)
        {
            total += this.options.NameWeight;
        }

        if (w.TypeLinePresent)
        {
            total += this.options.TypeLineWeight;
        }

        if (w.RulesTextPresent)
        {
            total += this.options.RulesTextWeight;
        }

        if (w.PowerToughnessPresent)
        {
            total += this.options.PowerToughnessWeight;
        }

        if (w.BottomMetadataPresent)
        {
            total += this.options.BottomMetadataWeight;
        }
